...
installed_major=""; IFS='.' read -r _1 installed_major _2 <<< "$installed_version"
latest_major=""; IFS='.' read -r _1 latest_major _2 <<< "$latest_version"
...
if [[ $latest_major -gt $installed_major ]]; then
	...
	for ((i = installed_major + 1; i <= latest_major; i++)); do
		...
		if [[ $? -ne 0 ]]; then
			...
			log INFO "Applied breaking changes patch ${GREEN}successfully${NC} for version $i."
		fi
	done
	log INFO ""
fi
